...
try:
    ...
    settings = None
    MODEL_CONFIG = {
        "openai_gpt4": {"type": "openai", "api_key": os.getenv("OPENAI_API_KEY", ""), "model": "gpt-4o"},
    }
    cache_manager = None
...
# ========== 多LLM主流程 ==========
@app.post("/llm/qa")
async def multi_llm_qa(req: QARequest, background_tasks: BackgroundTasks, authorization: str = Header(None)):
    verify_bearer(authorization)
    ...
    if cache_manager and settings and settings.enable_caching:
        cached_result = cache_manager.get_consensus_score(req.question, req.model_ids, req.roles)
        if cached_result:
    ...
    if req.method == "agreement":
        ...
        verdict = "一致性高，可信" if score >= threshold else "一致性较低，建议进一步链式验证"
        log["verdict"] = verdict
    ...
    if analytics_manager and QueryAnalytics:
        ...
        analytics = QueryAnalytics(
            ...
            success=True,
            individual_scores=dict(zip(req.model_ids, indiv_scores)) if 'indiv_scores' in locals() else None,
