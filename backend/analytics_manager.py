...
class AnalyticsManager:
            ...
            with self.lock:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    """, (
                        ...
                        json.dumps(analytics.individual_scores) if analytics.individual_scores else None,
        ...
        for model_id in analytics.model_ids:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    if row:
                        total_queries, avg_response_time, avg_consensus_score, success_rate, error_count = row
                        
                        # Update running averages
                        new_total = total_queries + 1
                        new_avg_time = ((avg_response_time * total_queries) + analytics.response_time) / new_total
                        new_avg_consensus = ((avg_consensus_score * total_queries) + analytics.consensus_score) / new_total
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (
                        model_id, new_total, new_avg_time, new_avg_consensus,
                        new_success_rate, error_count, analytics.timestamp.isoformat()
                    ))
                    
            except Exception as e:
                logger.error(f"Failed to update model performance for {model_id}: {e}")
    
    def get_model_performance(self, model_id: Optional[str] = None) -> List[ModelPerformance]:
    ...
    def get_query_analytics(self, 
                           ...
                           limit: int = 100) -> List[QueryAnalytics]:
    ...
    def cleanup_old_data(self):
        ...
        try:
            cutoff_date = datetime.now() - timedelta(days=settings.analytics_retention_days)
