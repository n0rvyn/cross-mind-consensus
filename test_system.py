...
def test_agreement_method():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Individual Scores: {result.get('individual_model_agreement', {})}")
...
def test_auto_weights():
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/llm/auto-weights",
            json=payload,
            headers=HEADERS
        )
...
def test_edge_cases():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE_URL}/llm/qa", json=payload, headers=HEADERS)
