...
if page == "📊 Dashboard":
    ...
    if health_data:
        ...
        with col1:
            st.metric(
                "System Status",
                "🟢 Online" if health_data.get("status") == "healthy" else "🔴 Offline",
            )
        ...
        with col2:
            ...
            st.metric(
                "Cache Status",
                "🟢 Active" if cache_info.get("enabled") else "🔴 Disabled",
            )
        ...
        if analytics_data:
            ...
            with col1:
                ...
                st.metric(
                    ...
                    f"{analytics_data.get('avg_response_time', 0):.2f}s",
                )

            with col2:
                st.subheader("🎯 Success Metrics")
                st.metric(
                    "Success Rate", f"{analytics_data.get('success_rate', 0):.1f}%"
                )
                st.metric(
                    "Model Combinations",
                    analytics_data.get("unique_model_combinations", 0),
                )
...
# Query Interface Page
    ...
    with st.form("query_form"):
        ...
        with col2:
            ...
            models_data = make_api_request("/models")
            available_models = (
                list(models_data.get("models", {}).keys()) if models_data else []
            )

            selected_models = st.multiselect(
                "Select Models", available_models, default=available_models[:3]
            )
            roles = st.text_input(
                "Roles (comma-separated)", "Expert,Reviewer,Validator"
            ).split(",")
        ...
        if submit_button and question and selected_models:
            with st.spinner("Processing query..."):
                ...
                if result:
                    ...
                    with col1:
                        st.metric(
                            "Consensus Score", f"{result.get('consensus_score', 0):.3f}"
                        )
                    with col2:
                    ...
                    for answer in result.get("answers", []):
                        with st.expander(f"{answer['model_id']} ({answer['role']})"):
                            ...
                            st.caption(
                                f"Individual Score: {result.get('individual_scores', {}).get(answer['model_id'], 0):.3f}"
                            )
                    ...
                    if "chain_process" in result:
                        ...
                        for round_data in result["chain_process"]:
                            with st.expander(f"Round {round_data['round']}"):
                                ...
                                st.write(
                                    "**Revised Answer:**", round_data["revised_answer"]
                                )
...
# Analytics Page
    ...
    if models_data and models_data.get("model_performances"):
        ...
        if not df.empty:
            ...
            display_df["avg_consensus_score"] = display_df["avg_consensus_score"].round(
                3
            )
...
# Model Management Page
    ...
    if models_data:
        ...
        for model_id, config in models_data["models"].items():
            with st.expander(
                f"{model_id} ({'✅ Available' if config['available'] else '❌ Unavailable'})"
            ):
                col1, col2 = st.columns(2)
                with col1:
...
# Batch Processing Page
    ...
    selected_models = st.multiselect(
        "Select Models for Batch", available_models, default=available_models[:2]
    )
    ...
    if st.button("🚀 Submit Batch") and batch_text and selected_models:
        ...
        if len(questions) > max_batch_size:
            ...
            with st.spinner("Processing batch..."):
                ...
                if result:
                    st.success(
                        f"Batch completed! Processed {result.get('successful_requests', 0)}/{result.get('total_requests', 0)} requests"
                    )
                    st.metric(
                    ...
                    for i, res in enumerate(result.get("results", [])):
                        if "error" not in res:
                            with st.expander(f"Question {i+1}: {questions[i][:50]}..."):
                                st.metric(
                                    "Consensus Score",
                                    f"{res.get('consensus_score', 0):.3f}",
                                )
...
# Cache Management Page
    ...
    if cache_stats:
        ...
        with col1:
            st.metric(
                "Cache Status",
                "🟢 Enabled" if cache_stats.get("enabled") else "🔴 Disabled",
            )
        with col2:
            st.metric("Memory Cache Size", cache_stats.get("memory_cache_size", 0))
        with col3:
            st.metric(
                "Embedding Cache Size", cache_stats.get("embedding_cache_size", 0)
            )
        ...
        if cache_stats.get("redis_available"):
            ...
            with col3:
                ...
                if (
                    ...
                    + cache_stats.get("redis_misses", 0)
                    > 0
                ):
                    hit_rate = (
                        cache_stats.get("redis_hits", 0)
...
# Real-time Monitor Page
    ...
    with real_time_container:
        if st.session_state.real_time_queries:
            ...
            st.info(
                "No real-time data available. Start some queries to see live updates!"
            )
